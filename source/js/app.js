...
if (
  (ie > 9 || typeof ie === 'undefined') &&
  (typeof ios === 'undefined' || ios >= 8)
) {
  ...
  hyperform.addTranslation('sv', {
    TextTooLong:
      'Använd %l eller färre tecken (du använder just nu %l tecken).',
    ...
    StepMismatch:
      'Värdet är felaktigt. De två närmsta godkända värdena är %l och %l.',
    StepMismatchOneValue:
      'Välj ett godkänt värde. Det närmsta godkända värdet är %l.',
...
if (postFilters && !disablePostFilterJs) {
  document.querySelector('.toogle').addEventListener('click', function (e) {
    ...
    var toogleElement = e.target.getAttribute('data-toogle');
    [].map.call(document.querySelectorAll(toogleElement), function (element) {
      element.classList.toggle('hidden');
    });
  });
  document
    .querySelector('#filter-keyword')
    .addEventListener('click', function (e) {
      var x = screenSize();
      if (x >= 895) {
        if (
          document
            .getElementById('show-date-filter')
        ...
        if (
          !document.getElementById('date-filter').classList.contains('hidden')
        ) {
